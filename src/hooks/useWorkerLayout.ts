...
export function useWorkerLayout(job: LayoutJob | null): WorkerLayoutState {
  const [state, setState] = useState<WorkerLayoutState>({
    layout: null,
    isLayouting: false,
  });
  ...
  // so an ok:false response can be retried synchronously on the main thread.
  ...
  useEffect(() => {
    if (!job) {
      setState({ layout: null, isLayouting: false });
      return;
    }

    const id = ++lastIdRef.current;

    const applyResult = (layout: LayoutJobResult) => {
      if (id !== lastIdRef.current) return; // superseded
      busySinceRef.current = null;
      setState({ layout, isLayouting: false });
    };

    const runSync = () => {
      try {
        applyResult(computeLayoutJob(job));
      } catch {
        // Malformed input mid-edit: keep the previous layout on screen.
        if (id === lastIdRef.current) {
          setState((prev) => ({ ...prev, isLayouting: false }));
        }
      }
    };
    ...
    try {
      if (!workerRef.current) {
        ...
        workerRef.current.onmessage = (e: MessageEvent<LayoutResponse>) => {
          ...
          busySinceRef.current = null;
          if (res.ok) {
            setState({ layout: res.layout, isLayouting: false });
            return;
          }
          // The worker can fail where the main thread succeeds (workers get a
          // smaller call stack and dagre recurses deeply on long chains) —
          // retry synchronously before giving up.
          const current = currentJobRef.current;
          try {
            if (!current) throw new Error(res.error);
            setState({ layout: computeLayoutJob(current), isLayouting: false });
          } catch {
            setState((prev) => ({ ...prev, isLayouting: false }));
          }
        };
      ...
      setState((prev) => ({ ...prev, isLayouting: true }));
