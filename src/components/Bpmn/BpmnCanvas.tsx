...
function roundedPath(points: Pt[]): string {
  ...
  for (let i = 1; i < points.length - 1; i++) {
    const prev = points[i - 1], c = points[i], next = points[i + 1];
    const inLen = dist(prev, c), outLen = dist(c, next);
    if (inLen < 0.5 || outLen < 0.5) {
      d += ` L ${c.x} ${c.y}`;
      continue;
    }
    const r = Math.min(CORNER, inLen / 2, outLen / 2);
    const a = lerp(c, prev, r / inLen), b = lerp(c, next, r / outLen);
...
function Arrow({ end, prev, color }: { end: Pt; prev: Pt; color: string }) {
  ...
  return <path d={`M ${end.x} ${end.y} L ${p1.x} ${p1.y} L ${p2.x} ${p2.y} Z`} fill={color} />;
}
